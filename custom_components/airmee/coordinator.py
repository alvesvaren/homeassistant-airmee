...
from .const import (
    DEFAULT_POLL_INTERVAL, SHORT_POLL_INTERVAL, SOON_THRESHOLD,
...
class AirmeeDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        ...
        if isinstance(data, list) and data:
            ...
            valid = [p for p in data if p.get("dropoff_eta") and int(p["dropoff_eta"]) >= now_ts]
            if valid:
                next_eta_ts = int(min(valid, key=lambda p: int(p["dropoff_eta"]))["dropoff_eta"])

        now_ts = int(datetime.now(tz=timezone.utc).timestamp())
        self.update_interval = (
            timedelta(seconds=SHORT_POLL_INTERVAL)
            if next_eta_ts and (next_eta_ts - now_ts) < SOON_THRESHOLD
            else timedelta(seconds=DEFAULT_POLL_INTERVAL)
        )
    ...
    async def async_refresh_token(self):
        ...
        session = async_get_clientsession(self.hass)
        headers = {"Content-Type": "application/json", "Authorization": "TEMP_TOKEN"}
        ...
        # persist to the config entry (correct API, do not await)
        updated = {**self.entry.data,
